...
def load_dpr_results(pred_results, top_n=5, split="train", align_dict=None):
    ...
    for item in tqdm(pred_results):
        ...
        q_c_a.append({"question": question, "answers": answers,
                      "context": context, "lang": lang, "qid": qid})
...
def main():
    ...
    if args.dev_fp is not None:
        ...
        #                 item["question"], item["lang"], item["context"]), item["answers"], item["qid"]])
        #         else:
        #             tsv_writer.writerow(["<Q>: {0} <P>:{1}".format(
        #                 item["question"], item["context"]), item["answers"]])
        with open(os.path.join(args.output_dir, "gold_para_qa_data_dev.jsonl"), "w") as out_file:
            jsonl_writer = jsonlines.Writer(out_file)
            for item in s2s_dev:
                jsonl_writer.write({
                    "prompt": "<Q>: {0} <P>:{1}".format(item["question"], item["context"]),
                    "answer": item["answers"]
                })
