...
def encode_line(tokenizer, line, max_length, padding_side, pad_to_max_length=True, return_tensors="pt"):
    extra_kw = {"add_prefix_space": True} if isinstance(
        tokenizer, BartTokenizer) and not line.startswith(" ") else {}
...
class Seq2SeqDataset(Dataset):
    ...
    def __getitem__(self, index) -> Dict[str, torch.Tensor]:
        ...
        if self.src_lang is not None:
            source_tokenizer.set_src_lang_special_tokens(self.src_lang)
        source_inputs = encode_line(
            source_tokenizer, source_line, self.max_source_length, "right")
        if self.tgt_lang is not None:
            target_tokenizer.set_tgt_lang_special_tokens(self.tgt_lang)
...
def get_scores(args, preds_path, gold_data_path):
    ...
    if args.gold_data_mode == "qa":
        with jsonlines.open(gold_data_path) as reader:
            for line in reader:
